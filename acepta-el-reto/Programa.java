...
/*
 * if (!DelDiag.contains(Posiciones.get(index).J + Posiciones.get(index).I)) {
 * Q--;
 * DelRow.add(Posiciones.get(index).I);
 * DelCol.add(Posiciones.get(index).J);
 * 
 * while (Posiciones.get(index).I + 1 < tablero.length ||
 * Posiciones.get(index).J + 1 < tablero.length) {
 * DelDiag.add(Posiciones.get(index).I + Posiciones.get(index).J);
 * }
 * 
 * while (Posiciones.get(index).I - 1 > 0 || Posiciones.get(index).J - 1 > 0) {
 * DelDiag.add(Posiciones.get(index).I + Posiciones.get(index).J);
 * }
...
public class Programa {
  ...
  public static void Torres_y_Reinas(int[][] tablero, int Q, int R) {
    int Final = 0;
    ...
    while (Q + R > 0 && index < tablero.length * tablero.length) {
      ...
      if (!DelRow.contains(Posiciones.get(index).I) && !DelCol.contains(Posiciones.get(index).J)) {
        ...
        // System.out.print(Posiciones.get(index).I);
        // System.out.print(Posiciones.get(index).J);
        // System.out.println();
        if (R > 0) {
           ...
           * 
           * int mover = 1;
           * while (Posiciones.get(index).I + mover < tablero.length &&
           * Posiciones.get(index).J + mover < tablero.length) {
           * 
           * if (tablero[Posiciones.get(index).I - mover][Posiciones.get(index).J - mover]
           * == -1) {
           * posible = false;
           * }
           * 
           ...
           * 
           * mover = 1;
           * while (Posiciones.get(index).I - mover >= 0 && Posiciones.get(index).J -
           * mover >= 0) {
           * 
           * if (tablero[Posiciones.get(index).I - mover][Posiciones.get(index).J - mover]
           * == -1) {
           ...
           * mover = 1;
           * while (Posiciones.get(index).I + mover < tablero.length &&
           * Posiciones.get(index).J - mover >= 0) {
           * 
           * if (tablero[Posiciones.get(index).I - mover][Posiciones.get(index).J - mover]
           * == -1) {
           * posible = false;
